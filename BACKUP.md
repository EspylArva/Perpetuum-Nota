...
|------|----------------|----------|
| PostgreSQL | volume `perpetuum-nota_pgdata` (service `db`) | users, notes (incl. content JSON + search text), tags, shares, image metadata |
| Uploads | volume `perpetuum-nota_uploads` (service `api`, mounted at `/data/uploads`) | the image files themselves |
...
docker compose exec -T db pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Fc \
  > "backup/perpetuum-nota-$(date +%F).dump"
```
...
```bash
docker run --rm \
  -v perpetuum-nota_uploads:/data:ro \
  -v "$(pwd)/backup:/backup" \
...
```cron
30 3 * * * cd /path/to/perpetuum-nota && \
  docker compose exec -T db pg_dump -U perpetuum_nota -d perpetuum_nota -Fc > backup/db-$(date +\%F).dump && \
  docker run --rm -v perpetuum-nota_uploads:/data:ro -v $(pwd)/backup:/backup alpine \
...
docker compose exec -T db pg_restore -U perpetuum_nota -d perpetuum_nota --clean --if-exists \
  < backup/perpetuum-nota-YYYY-MM-DD.dump

# 3. restore uploads
docker run --rm \
  -v perpetuum-nota_uploads:/data \
  -v "$(pwd)/backup:/backup" \
